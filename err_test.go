...
func TestNewErr_reasonIsValue(t *testing.T) {
	err := re.NewErr(InvalidValue{Value: "abc"})
...
func TestNewErr_withCause(t *testing.T) {
	...
	err := re.NewErr(InvalidValue{Value: "abc"}, cause)
...
func TestNewErr_causeIsAlsoErr(t *testing.T) {
	cause := re.NewErr(FailToGetValue{Name: "foo"})
	err := re.NewErr(InvalidValue{Value: "abc"}, cause)
...
func TestErr_IfOk_ok(t *testing.T) {
	...
	s := ""
	err2 := err.IfOk(func() re.Err {
		s = "executed."
		return re.NewErr(InvalidValue{Value: "x"})
	})
...
func TestErr_IfOk_err(t *testing.T) {
	err := re.NewErr(InvalidValue{Value: "x"})
