...
func ExampleNewErr() {
	...
	err := reasonederror.NewErr(FailToDoSomething{})
	fmt.Printf("(1) %v\n", err)

	// (2) Creates an Err with situation parameters.
	err = reasonederror.NewErr(FailToDoSomethingWithParams{
		Param1: "ABC",
		Param2: 123,
	})
	fmt.Printf("(2) %v\n", err)
	...
	err = reasonederror.NewErr(FailToDoSomething{}, cause)
	fmt.Printf("(3) %v\n", err)
...
func ExampleErr_Reason() {
	...
	err := reasonederror.NewErr(FailToDoSomething{Param1: "value1"})
	...
	err = reasonederror.NewErr(&FailToDoSomething{Param1: "value2"})
	switch err.Reason().(type) {
	case *FailToDoSomething:
		fmt.Println("The reason of the error is: *FailToDoSomething")
		reason := err.Reason().(*FailToDoSomething)
...
func ExampleErr_Unwrap() {
	...
	err := reasonederror.NewErr(FailToDoSomething{}, cause1)
...
func ExampleErr_IfOk() {
	...
	err := reasonederror.Ok()
	err.IfOk(func() reasonederror.Err {
